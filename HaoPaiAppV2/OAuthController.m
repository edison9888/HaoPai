...
- (void) gotPin: (NSString *) pin {
	...
	[self performSelector:@selector(setSync) withObject:nil afterDelay:0.0];
}

-(void)setSync{
	NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
	NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:kRESTBaseUrl,@"passport.setUserSyncInfo"]];
	ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
	[request setPostValue:[defaults objectForKey:@"sessionkey"] forKey:@"sessionkey"];
	//NSLog(@"userid %@",[defaults objectForKey:@"userid"]);
	//[request setPostValue:[defaults objectForKey:@"userid"] forKey:@"user_id"];
	[request setPostValue:@"3" forKey:@"site_id"];
	[request setPostValue:_engine.accessToken.key forKey:@"oauth_token"];
	[request setPostValue:_engine.accessToken.secret forKey:@"oauth_secret"];
...
- (void) loadView {
	...
	//默认情况下，一个web view会自动的将出现在网页内容中的电话号码转换为电话链接。点击电话链接时，电话程序启动并且呼叫响应号码，
	//设置属性detectsPhoneNumbers为NO可以关闭默认的检测
