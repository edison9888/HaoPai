...
#import "SheetCamerViewController.h"
#import "UserLoginViewController.h"

static CGFloat const TBKTabBarDefaultStyleHeight = 57.0;
...
-(void) loadView {
    ...
    self.viewControllers = [NSArray arrayWithObjects:
                            [[[UINavigationController alloc] initWithRootViewController:[kAppDelegate loadFromVC:@"HomeViewController"]] autorelease],
                            [[[UINavigationController alloc] initWithRootViewController:[kAppDelegate loadFromVC:@"SearchStartViewController"]] autorelease],
                            [kAppDelegate loadFromVC:@"SheetCamerViewController"],
                            [[[UINavigationController alloc] initWithRootViewController:[kAppDelegate loadFromVC:@"FoodStartViewController"]] autorelease],
                            [[[UINavigationController alloc] initWithRootViewController:[kAppDelegate loadFromVC:@"UserProfileViewController"]] autorelease],
                            nil];
   // NSLog(@"tbk loadView");
}

-(void) viewDidLoad {
  //  NSLog(@"tbk viewDidLoad");
	[super viewDidLoad];
...
-(void) loadViewControllers {
	...
	for (UIViewController *controller in self.viewControllers) {
        ...
        //自定相机按钮
		if ([controller isMemberOfClass:[SheetCamerViewController class]]) {
			UIButton *tabItem = [UIButton buttonWithType:UIButtonTypeCustom];
			[tabItem setImage:[UIImage imageNamed:@"main-photo"] forState:UIControlStateNormal];
			[tabItem setImage:[UIImage imageNamed:@"main-photo-b"] forState:UIControlStateHighlighted];
...
-(void)presentCamer{
	...
	if (TTIsStringWithAnyText([defaults objectForKey:@"sessionkey"])) {
		...
		UserLoginViewController *controller = (UserLoginViewController*)[kAppDelegate loadFromVC:@"UserLoginViewController"];
		//controller.delegate = self;
		//controller.stringSelect = @"presentCamer";
...
-(void) tabBar:(TBKTabBar *)aTabBar didSelectTabAtIndex:(NSUInteger)anIndex {
    ...
    if ([vc isMemberOfClass:[SheetCamerViewController class]]) {
		[self performSelector:@selector(presentCamer)
				   withObject:nil
				   afterDelay:0.1];
